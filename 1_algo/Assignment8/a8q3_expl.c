...
int knapsackDP(int W, int wt[], int val[], int n) {
  ...
  // Dynamically allocate a 2D array K[n+1][W+1]
...
int main() {
  ...
  while (1) {
    ...
    if (choice >= 1 && choice <= 3) {
      ...
      case 3:
        // Run DP first for immediate feedback
        start = clock();
        ...
        // Skip recursive if n is too large to prevent system freeze
        if (n > 25) {
          printf("Skipping Recursive test. Number of packages is too large "
                 "(>25) and will take an excessive amount of time.\n");
        } else {
          start = clock();
