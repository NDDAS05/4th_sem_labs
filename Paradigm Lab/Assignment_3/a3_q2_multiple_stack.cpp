...
int main() {
  ...
  bool stack1Init = false;
  bool stack2Init = false;
  bool *currentInit = nullptr;
  ...
  while (true) {
    ...
    if (mainChoice == 1) {
      current = &stack1;
      currentInit = &stack1Init;
    } else if (mainChoice == 2) {
      current = &stack2;
      currentInit = &stack2Init;
    } else
      continue;
    ...
    while (true) {
      ...
      if (!(*currentInit) && choice != 1) {
        std::cout << "Error: You must initialize the stack first.\n";
        continue;
      }
