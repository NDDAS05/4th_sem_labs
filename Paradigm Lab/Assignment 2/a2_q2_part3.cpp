...
int main() {
  ...
  if (size == 0 || size == -1)
    ...
    if (limit == 0 || limit == -1)
      ...
      stackLib::initialize(stack1, size, limit);
  }
  ...
  if (size == 0 || size == -1) // Default
    ...
    if (limit == 0 || limit == -1)
      ...
      stackLib::initialize(stack2, size, limit);
  }
  ...
  while (true) {
    ...
    if (choice == 1) {
      ...
      if (!(stackLib::push(stack1, value))) {
        cout << "\nStack 1 is full. Auto-inserting " << value
             << " into Stack 2 and moving to Stack 2 input." << endl;

        if (!(stackLib::push(stack2, value))) {
          cout << "Stack 2 is also full!" << endl;
        }
        break;
      }
    } else if (choice == 0) {
  ...
  while (true) {
    ...
    if (choice == 1) {
      ...
      if (!(stackLib::push(stack2, value))) {
        cout << "No more elements can be pushed. Executing remaining part.\n"
             << endl;
        break;
      }
    } else if (choice == 0) {
  ...
  if (size > 100)
    stackLib::initialize(stack3, size, size * 2);
  else
    stackLib::initialize(stack3, size);
