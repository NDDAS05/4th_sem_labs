...
template <typename T> void initialize(stk<T> &s, int size = 2, int limit = 10) {
  s.capacity = size;
  s.MaxLimit = limit;
  s.top = -1;
...
template <typename T> void release(stk<T> &s) {
  delete[] s.stack_arr;
  s.stack_arr = nullptr;
  s.top = -1;
}

template <typename T> int inflate(stk<T> &s) {
...
template <typename T> int push(stk<T> &s, const T &value) {
...
template <typename T> int push(stk<T> &s, T *arr, int count) {
...
template <typename T> T pop(stk<T> &s) {
  if (s.top == -1) {
...
template <typename T> void display(const stk<T> &s) {
  std::cout << "[ ";
  for (int i = 0; i <= s.top; i++)
...
int main() {
  ...
  while (true) {
    ...
    if (ch == 1) {
      ...
      stackLib::stk<int> s;
      stackLib::initialize(s, (size <= 0 ? 2 : size),
                           (limit <= 0 ? 10 : limit));
      ...
      while (true) {
        ...
        case 1:
          ...
          break;
        case 2:
          std::cout << "Popped: " << stackLib::pop(s) << std::endl;
          break;
        case 3:
          stackLib::display(s);
          break;
      ...
      while (true) {
        ...
        case 1: {
          ...
          char *newStr = new char[strlen(buffer) + 1];
          strcpy(newStr, buffer);

          if (!stackLib::push(s, newStr)) {
            delete[] newStr;
          }
          break;
        }
        case 2: {
          char *poppedStr = stackLib::pop(s);
          if (poppedStr != nullptr) {
            std::cout << "Popped: " << poppedStr << std::endl;
            delete[] poppedStr;
          }
          break;
        }
